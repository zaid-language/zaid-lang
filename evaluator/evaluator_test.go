...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{"5 + true", "1:3:test.zaid: runtime error: type mismatch: NUMBER + BOOLEAN"},
		{"5 + true; 5", "1:3:test.zaid: runtime error: type mismatch: NUMBER + BOOLEAN"},
		{"-true", "1:1:test.zaid: runtime error: unknown operator: -BOOLEAN"},
		{"true + false", "1:6:test.zaid: runtime error: unknown operator: BOOLEAN + BOOLEAN"},
		{"5; true + false; 5", "1:9:test.zaid: runtime error: unknown operator: BOOLEAN + BOOLEAN"},
		{"if (10 > 1) { if (10 > 1) { return true + false } return 1 }", "1:41:test.zaid: runtime error: unknown operator: BOOLEAN + BOOLEAN"},
		{"foobar", "1:1:test.zaid: runtime error: unknown identifier: foobar"},
		{`"Hello" - "World"`, "1:9:test.zaid: runtime error: unknown operator: STRING - STRING"},
		{`{"name": "Zaid"}[function() { 123 }]`, "1:17:test.zaid: runtime error: unusable as map key: FUNCTION"},
		{`function foo() { a } foo()`, "1:18:test.zaid: runtime error: unknown identifier: a"},
		{`class Test { function foo() { a } } test = Test.new() test.foo()`, "1:31:test.zaid: runtime error: unknown identifier: a"},
	}
...
func TestForExpressions(t *testing.T) {
	tests := []struct {
		...
		{`x = 10; for (x = y; x > 0; x = x - 1) { x }`, "1:18:test.zaid: runtime error: unknown identifier: y"},
		{`for (x = 0; x < 10; x = x + 1) { y }`, "1:34:test.zaid: runtime error: unknown identifier: y"},
		{`bar = true; for (x = 0; x < 10; x = x + 1) { y; printftw(bar) }`, "1:46:test.zaid: runtime error: unknown identifier: y"},
	}
...
func TestForInExpressions(t *testing.T) {
	tests := []struct {
		...
		{`list = [1, 2, 3]; for(x in lists) { x }`, "1:28:test.zaid: runtime error: unknown identifier: lists"},
	}
